...
const saveBrainDump = (data, taskId, userActions, content1, content2, group, user) => {
...
const initiateBraindump = (imageLocation, user, userActions, content1, content2, group, taskId) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			await saveBrainDump({ imageLocation: imageLocation }, taskId, userActions, content1, content2, group, user);
...
export const submitScreenShot = (taskId, content1, content2, group, user, userActions, uri, type) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (result) {
				await initiateBraindump(result.Location, user, userActions, content1, content2, group, taskId);
			}
			resolve(true);
		} catch (error) {
			reject(error);
